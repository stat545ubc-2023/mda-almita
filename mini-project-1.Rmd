...
steam_data <- steam_games %>%
  ...
  select(name, genre, release_date, release_month, release_year, all_reviews, developer, publisher, original_price, discount_price, url)
```
To be able to order months chronologically, we can convert `release_month` from string to an ordered factor column. 
...
I'd like to explore the distribution of game prices across steam, so I'll make a density plot to visualize it. But before that I want to explore a bit more the price column:

```{r}
summary(steam_data$original_price)
```
So the max price seems a bit outrageous, and there is a large number of games with NA's for price, I want to see what are the entries for both of these cases (extremely expensive and `original_price == NA`).

```{r}
steam_data %>%
  filter(original_price > 10000)
```

So I looked this game up and the price doesn't match up, its 14.99 USD on Steam so I'll just replace the wrong price with the real one.
...
```
So from looking some of these games up they're not free games, at least not all, so for simplicties sake I'll drop them...
...
There seem to be some games above the 500 price-point that act as outliers, not allowing to see the bulk of the most common prices. 

```{r}
steam_data %>% filter(original_price > 500)
```

Most of these games have repeated prices, and they also appear to be DLCs not games themselves, maybe the data is not as clean as I thought it was... Again, for simplicities sake I'll remove those entries.
...
steam_data %>%
  ggplot(aes(original_price)) +
  geom_density(aes(y=after_stat(count)), fill = "pink", colour = "pink", alpha = 0.5) +
  scale_x_continuous(trans = scales::pseudo_log_trans(base = 10), labels = scales::dollar_format(), breaks = c(0,1,2,5,10,20,50,100,200,400)) +
  ylab("Number of games") +
...
I'm interested especially in exploring and analyzing video game genre. I didn't explore it in this milestone because I'd have to keep myself from answering all the research questions now and have nothing to do in the next milestone.😅
