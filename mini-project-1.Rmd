...
I'd like to explore the distribution of game prices across steam, so I'll make a density plot to visualize it. First I need to remove `NA` values, then make the plot

```{r}
min(steam_games$original_price, na.rm = TRUE)
max(steam_games$original_price, na.rm = TRUE)
```

```{r}
steam_games %>%
  filter(!is.na(original_price)) %>%
  ggplot(aes(x = log10(original_price + 1), na.rm = TRUE)) +
  geom_density()
```
