...
#### How has genre popularity changed across time?

##### Summarising

*(2) Compute the number of observations for at least one of your categorical variables.*
For this task I will determine the number of observations of the following variables:
- `genres`
- `steamspy_tags`
...
For this task I want to create a stream chart using the `ggstream` package for `ggplot`. 

```{r}
library(ggstream)
```

I will probably only graph the top n genres and tags for better visualization. For that I will create a vector to filter with.
...
```{r}
pal = c("#17154FFF", "#2F357CFF", "#6C5D9EFF", "#9D9CD5FF", "#B0799AFF", "#F6B3B0FF", "#E48171FF", "#BF3729FF", "#E69B00FF", "#F5BB50FF", "#ADA43BFF", "#355828FF")
...
steam_separate %>%
  ...
  ggplot(aes(release_year, percentage, fill = genres)) +
...
steam_separate %>%
  ...
  reframe(number_of_games = n()) %>%
  ggplot(aes(release_year, number_of_games, fill = genres)) +
  geom_stream() +
  theme_minimal()
```
