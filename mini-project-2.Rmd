...
To determine the popularity of each genre or tags across time we first need to determine the number of observations of each variable. For this task I will determine the number of observations of the following variables:
- `genres`
- `steamspy_tags`
...
Now to construct the graphs: 
TODO explain why I used percentage and not counts
TODO separate the tibble creation from the graphing
...
3. Filter out genres and tags such as `Early Access`, `Free to Play`, and `Indie` , because they don't specify the **type** of game. For example, `Indie` will be applied to any small developer regardless of the type of game it is. 
...
steam_rq2 <- steam_rq1 %>%
  ...
  filter(!genres %in% c("Early Access", "Indie") & !steamspy_tags %in% c("Early Access", "Indie") & !steamspy_tags %in% genres)
...
First lets figure out how many developers there are:
...
I'm thinking that I probably will need to select the number of developers to display, if not then the graph will be too dense to read.

```{r}
length(unique(dev_genres$developer))
length(unique(dev_tags$developer))
```

Indeed, that is way too many developers, and that doesn't even consider that some games have multiple developers. So I will need to cutoff probably based on number of games released, visualizing the developers that have more games will probably answer better my research question. To decide the cutoff point I will make a histogram to see the number of games per developer distribution.

First we will separate the rows that have multiple developers. Note that I'm doing this in the `steam` dataset and not `steam_rq1` because in `steam` each observation corresponds to a single game, whereas in `steam_rq1` we have repeated entries for each genre or tag a game has. However I will have to separate the developers later on when making the final graph. 

```{r}
steam_rq1_devs <- steam %>%
  separate_rows(developer, sep = ";")
...
dev_count %>%
  ...
  scale_y_continuous(trans = scales::log1p_trans(),
                     breaks = c(1, 2, 5, 10, 50, 100, 250, 500, 1000, 2500, 5000, 10000, 15000, 20000)) +
...
So most of developers have only released 1 game, keeping them wouldn't make sense in the context of answering my research question. I will filter them out and see how many there are now.
...
That is still too many, therefore I think the best strategy is to pick the 20 developers who have released the most games. Note that I previously ordered the dev_count tbl with the `count` function.
...
print(top_devs)
```
Now that we got our top devs we can continue with the graphing.

```{r}
steam_rq1 %>%
  separate_rows(developer, sep = ";") %>%
  filter(developer %in% top_devs) %>%
