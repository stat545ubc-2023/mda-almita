...
2.  do developers tend to stick to the same genres?
...
```{r}
steam_separate <- steam %>%
  separate_rows(genres, sep = ";") %>%
  separate_rows(steamspy_tags, sep = ";") 

unique(steam_separate$genres)
unique(steam_separate$steamspy_tags)[1:20]
```
...
The data that helps respond the research question is also obtained the same way:
...
```{r}
top_genres <- steam_separate %>%
  count(genres, sort = TRUE) %>%
  slice_head(n = 11) %>%
  pull(genres)

top_tags <- steam_separate %>%
  count(steamspy_tags, sort = TRUE) %>%
  slice_head(n = 16) %>%
  pull(steamspy_tags)
...
```{r}
top_unique_tags <- steam_separate %>%
...
#### RQ2 - Do developers tend to stick to the same genres?
